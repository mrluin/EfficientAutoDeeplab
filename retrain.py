...
def main(args):
    ...
    if args.resume_from_retrain:
        ...
        args.__dict__.update(config_dict) # update configs from the last retrain phase.
    ...
    else: # resume partial configs setting and arch_checkpoint from the search phase by default.
        #assert os.path.exists(args.checkpoint_file), 'cannot find checkpoint_file from the search phase: {:}'.format(args.checkpoint_file)
        ...
        f = open(config_file_path, 'r')
        random_seed = json.load(f)['random_seed']
        args.random_seed = random_seed
        # don't need to resume configs from search phase.
        EXP_time = args.resume_file.split('/')[-1] # the same EXP_time with the time used in search phase.
        args.path = os.path.join(args.path, args.exp_name, EXP_time)
        # save re-train configs, when resume from search phase
        save_configs(args.__dict__, args.path, 'retrain')
        logger = prepare_logger(args)
        logger.log('=> starting retrain from the search phase config {:}.'.format(config_file_path))
    ...
    if args.open_test == False: # retrain and validate
        ...
        if args.resume_from_retrain:
            #assert os.path.exists(args.resume_file), 'cannot find resume_file {:} from the last retrain phase'.format(args.resume_file)
            logger.log('Loading checkpoint from {:} of the last retrain phase'.format(args.resume_file))
            ...
            logger.log('loaded checkpoint file {:} from the last retrain phase, starts with {:}-th epoch'.format(checkpoint_path, checkpoint['start_epoch']))
        else:
            # from the search phase, read arch_checkpoint
            #assert os.path.exists(args.checkpoint_file), 'cannot find checkpoint file {:} from the search phase.'.format(args.checkpoint_file)
            ...
            actual_path, cell_genotypes = checkpoint['actual_path'], checkpoint['cell_gentypes']
            new_genotypes = []
            ...
            normal_network.load_state_dict(checkpoint['state_dict'])
            display_all_families_information(args, 'retrain', retrain_run_manager, logger)
            logger.log('Construct NewGumbelAutoDeeplab according to the best-arch obtained from search phase', mode='info')
        # perform train and validation in train() method
        retrain_run_manager.train()
    else: # test phase
        #assert os.path.exists(args.resume_file), 'cannot find resume_file {:} from the retrain phase'.format(args.resume_file)
