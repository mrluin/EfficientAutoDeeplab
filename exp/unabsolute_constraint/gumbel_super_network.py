...
class GumbelAutoDeepLab(MyNetwork):
    def __init__(self,
                 ...
                 nb_layers, bn_momentum, bn_eps, search_space, logger, alpha_constraint=0.2 ,affine=False):
        ...
        # alpha constraint :: reduce importance of some path in fabric, in contrast to pruning the useless path.
        ...
        # TODO:
        # 1. change prev_prev_scale and prev_scale of the first two cells in each stage
        # 2. change cell architecture, the relationship of prev_prev_ and scale,
        # 3. add self.authors flag to control

        # cells
        prev_prev_c = 16 # output of stem1
        prev_c = 32 # output of stem2
        # cell in layer0 scale0
        prev_prev_scale = -2
        prev_scale = -1
        for i in range(self.nb_layers):
            if i == 0:
                # 4-1
                cell1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=0, search_space=self.search_space, ppc=prev_prev_c, pc=prev_c, affine=affine)
                # 8-1
                cell2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, ppc=prev_prev_c, pc=prev_c, affine=affine)
                self.cells += [cell1]  # 0
                self.cells += [cell2]  # 1
            elif i == 1:
                # 4-2
                cell1_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=0, search_space=self.search_space, ppc=prev_c, affine=affine)
                cell1_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=0, search_space=self.search_space, ppc=prev_c, affine=affine)
                # 8-2
                cell2_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, ppc=prev_c, affine=affine)
                cell2_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, ppc=prev_c, affine=affine)
                # 16-1
                cell3 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, ppc=prev_c, affine=affine)
                self.cells += [cell1_1]
                self.cells += [cell1_2]
                self.cells += [cell2_1]
                self.cells += [cell2_2]
                ...
                cell1_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=0, search_space=self.search_space, affine=affine)
                cell1_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=0, search_space=self.search_space, affine=affine)
                cell2_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, affine=affine)
                cell2_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, affine=affine)
                cell2_3 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=1, search_space=self.search_space, affine=affine)
                # 16-2
                cell3_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, affine=affine)
                cell3_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, affine=affine)
                # 32-1
                cell4 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=3, search_space=self.search_space, affine=affine)
                self.cells += [cell1_1]
                self.cells += [cell1_2]
                self.cells += [cell2_1]
                self.cells += [cell2_2]
                ...
                cell3_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, affine=affine)
                cell3_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, affine=affine)
                cell3_3 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=2, search_space=self.search_space, affine=affine)
                # 32-2
                cell4_1 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=3, search_space=self.search_space, affine=affine)
                cell4_2 = GumbelCell(i, self.filter_multiplier, self.block_multiplier, self.steps, scale=3, search_space=self.search_space, affine=affine)
                self.cells += [cell1_1]
                self.cells += [cell1_2]
                self.cells += [cell2_1]
                self.cells += [cell2_2]
        ...
        self.aspp4 = ASPP(scale4_outc, self.nb_classes, 24, affine=affine)
        self.aspp8 = ASPP(scale8_outc, self.nb_classes, 12, affine=affine)
        self.aspp16 = ASPP(scale16_outc, self.nb_classes, 6, affine=affine)
        self.aspp32 = ASPP(scale32_outc, self.nb_classes, 3, affine=affine)
    ...
    def calculate_entropy(self, single_path, eps=1e-8):
        ...
        for layer in range(self.nb_layers):
            ...
            cell_entropy =  - torch.sum(torch.mul(cell_probs, cell_log_probs)) #/ torch.log(torch.tensor(len(self.conv_candidates), dtype=torch.float))
    ...
    def single_path_forward(self, x, single_path):
        ...
        if last_scale == 0:
            ...
            aspp_result = self.aspp8(inter_features[-1])
        elif last_scale == 2:
            aspp_result = self.aspp16(inter_features[-1])
        elif last_scale == 3:
            aspp_result = self.aspp32(inter_features[-1])
        else:
            raise ValueError('invalid last_scale value {}'.format(last_scale))
    ...
    def _single_path_gdas_weightsum(self, layer, current_scale, next_scale, cell_index, index, hardwts, prev_prev_feature,
                                    prev_feature):
    ...
    def get_aspp_hardwts_index(self):
        while True:
            # shape as []
    ...
    def get_network_arch_hardwts_with_constraint(self):
        ...
        #
        # TODO: with alpha_constraint instead of absolutely constraint.

        # TODO: need test, whether network_arch_parameters update in this way
        alpha_arch_parameters = self.network_arch_parameters.clone()
        for layer in range(self.nb_layers):
            if layer >= 1 and layer <= 8:
                alpha_arch_parameters[layer][0][2] = alpha_arch_parameters[layer][0][2] * self.alpha_constraint
            if layer >= 2 and layer <= 8:
                alpha_arch_parameters[layer][1][2] = alpha_arch_parameters[layer][1][2] * self.alpha_constraint
            if layer >= 3 and layer <= 8:
                alpha_arch_parameters[layer][2][2] = alpha_arch_parameters[layer][2][2] * self.alpha_constraint
            if layer > 8:
                alpha_arch_parameters[layer][3][0] = alpha_arch_parameters[layer][3][9] * self.alpha_constraint
                alpha_arch_parameters[layer][1][0] = alpha_arch_parameters[layer][1][0] * self.alpha_constraint
                alpha_arch_parameters[layer][2][0] = alpha_arch_parameters[layer][2][0] * self.alpha_constraint
        ...
        while True:
            ...
            for layer in range(self.nb_layers):
                if layer == 0:
                    logits[layer][0][1] = \
                        (alpha_arch_parameters[layer][0][1].log_softmax(dim=-1) * (1 / 3)  + gumbels[layer][0][1]) / self.tau
                    probs[layer][0][1] = F.softmax(logits.clone()[layer][0][1], dim=-1) * (1 / 3)
                    logits[layer][1][0] = \
                        (alpha_arch_parameters[layer][1][0].log_softmax(dim=-1) * (1 / 3) + gumbels[layer][1][0]) / self.tau
                    ...
                    probs[layer][0][1] = F.softmax(logits.clone()[layer][0][1], dim=-1) * (1 / 3)
                    logits[layer][1][:2] = \
                        (alpha_arch_parameters[layer][1][:2].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][1][:2]) / self.tau
                    probs[layer][1][:2] = F.softmax(logits.clone()[layer][1][:2], dim=-1) * (2 / 3)
                    logits[layer][2][0] = \
                        (alpha_arch_parameters[layer][2][0].log_softmax(dim=-1) * (1 / 3) + gumbels[layer][2][0]) / self.tau
                    ...
                    probs[layer][1][:2] = F.softmax(logits.clone()[layer][1][:2], dim=-1) * (2 / 3)
                    logits[layer][2][:2] = \
                        (alpha_arch_parameters[layer][2][:2].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][2][:2]) / self.tau
                    probs[layer][2][:2] = F.softmax(logits.clone()[layer][2][:2], dim=-1) * (2 / 3)
                    logits[layer][3][0] = \
                        (alpha_arch_parameters[layer][3][0].log_softmax(dim=-1) * (1 / 3) + gumbels[layer][3][0]) / self.tau
                    ...
                    if layer <= 8:
                        ...
                        probs[layer][2][:2] = F.softmax(logits.clone()[layer][2][:2], dim=-1) * (2 / 3)
                        logits[layer][3][:2] = \
                            (alpha_arch_parameters[layer][3][:2].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][3][:2]) / self.tau
                        probs[layer][3][:2] = F.softmax(logits.clone()[layer][3][:2], dim=-1) * (2 / 3)
                    else:
                        logits[layer][0][1:] = \
                            (alpha_arch_parameters[layer][0][1:].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][0][1:]) / self.tau
                        probs[layer][0][1:] = F.softmax(logits.clone()[layer][0][1:], dim=-1) * (2 / 3)
                        logits[layer][1][1:] = \
                            (alpha_arch_parameters[layer][1][1:].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][1][1:]) / self.tau
                        probs[layer][1][1:] = F.softmax(logits.clone()[layer][1][1:], dim=-1) * (2 / 3)
                        logits[layer][2][1:] = \
                            (alpha_arch_parameters[layer][2][1:].log_softmax(dim=-1) * (2 / 3) + gumbels[layer][2][1:]) / self.tau
                        probs[layer][2][1:] = F.softmax(logits.clone()[layer][2][1:], dim=-1) * (2 / 3)
                        logits[layer][3][1] = \
                            (alpha_arch_parameters[layer][3][1].log_softmax(dim=-1) * (1 / 3) + gumbels[layer][3][1]) / self.tau
