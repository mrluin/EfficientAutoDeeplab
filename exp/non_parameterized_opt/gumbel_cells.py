...
class GumbelCell(MyModule):
    def __init__(self, layer,
        ...
        # TODO: in non-parameterized reduction and increase operations
        # 1. Reduction :: AveragePooling with stride2, DoubleReduction :: AveragePooling with stride4
        # 2. same :: ConvLayer 1x1
        # 3. Increase :: Interpolation with factor2, DoubleIncrease :: Interpolation with factor4
        ...
        if self.scale == 0:
            ...
            self.up_link_prev = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.up_link_prev_prev = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.double_up_link_prev_prev = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
            self.down_link_prev_prev = nn.MaxPool2d(3, 2, 1)
            ...
            self.down_link_prev = nn.MaxPool2d(3, 2, 1)
            self.up_link_prev = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.down_link_prev_prev = nn.MaxPool2d(3, 2, 1)
            self.up_link_prev_prev = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.double_up_link_prev_prev = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
            self.double_down_link_prev_prev = nn.MaxPool2d(3, 4, 1)
