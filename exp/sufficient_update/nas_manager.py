...
class ArchSearchConfig:
    ...
    def get_update_schedule(self, iter_per_epoch):
        ...
        for i in range(iter_per_epoch):
            if (i+1) % self.arch_param_update_frequency == 0:
...
class ArchSearchRunManager:
    ...
    def warm_up(self, warmup_epochs):
        ...
        for epoch in range(self.warmup_epoch, warmup_epochs):
            ...
            single_path = self.net.sample_single_path(self.run_manager.run_config.nb_layers, network_index, aspp_index)
            ...
            for i, (datas, targets) in enumerate(data_loader):
                ...
                if not fix_net_weights:
                    ...
                    if (i + 1) % self.arch_search_config.sample_arch_frequency == 0:
                        _, network_index = self.net.get_network_arch_hardwts_with_constraint()
                        _, aspp_index = self.net.get_aspp_hardwts_index()
                        single_path = self.net.sample_single_path(self.run_manager.run_config.nb_layers, network_index,
                                                                  aspp_index)
                    ...
                    # TODO: related to update_frequency and update_times
                    #for j in range(self.update_scheduler[i]): # at the i-th iteration, update arch_parameters update_scheduler[i] times.
                    ...
                    if (i+1) % self.arch_search_config.arch_param_update_frequency == 0 or (i+1) == iter_per_epoch:
