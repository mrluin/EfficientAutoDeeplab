...
class _LRScheduler(object):
    ...
    def update(self, cur_epoch, cur_iter):
        ...
        if cur_iter is not None:
            assert isinstance(cur_iter, int) and cur_iter >= 0, 'invalid cur-iter: {:}'.format(cur_iter)
            self.current_iter = cur_iter
        for param_group, lr in zip(self.optimizer.param_groups, self.get_lr()):
            param_group['lr'] = lr

class CosineAnnealingLR(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, epochs, T_max, eta_min, **kwargs):
        self.T_max = T_max
...
class MultiStepLR(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, epochs, milestones, gammas, **kwargs):
...
class ExponentialLR(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, epochs, gamma, **kwargs):
        self.gamma = gamma
...
class LinearLR(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, epochs, max_LR, min_LR, **kwargs):
        self.max_LR = max_LR
        self.min_LR = min_LR
...
class CrossEntropyLabelSmooth(nn.Module):
    def __init__(self, num_classes, epsilon, **kwargs):
        super(CrossEntropyLabelSmooth, self).__init__()
        # normal CELoss when epsilon is 0
    ...
    def forward(self, inputs, targets):
        ...
        loss = (-targets * log_probs).mean(0).sum(0)
        return loss
